...
function JobViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<JobInterface>(
    ...
    () =>
      getJobById(id, {
        relations: ['user_job_buyer_idTouser', 'user_job_seller_idTouser', 'review'],
      }),
  );
            ...
            <Box>
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Job Buyer Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_job_buyer_idTouser?.id}`}>
                      {data?.user_job_buyer_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Job Seller Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_job_seller_idTouser?.id}`}>
                      {data?.user_job_seller_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/reviews/create?job_id=${data?.id}`}>
