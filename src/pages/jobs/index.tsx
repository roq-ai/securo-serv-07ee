...
function JobListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<JobInterface[]>(
    ...
    () =>
      getJobs({
        relations: ['user_job_buyer_idTouser', 'user_job_seller_idTouser', 'review.count'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_job_buyer_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_job_seller_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_job_buyer_idTouser?.id}`}>
                          {record.user_job_buyer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_job_seller_idTouser?.id}`}>
                          {record.user_job_seller_idTouser?.email}
                        </Link>
                      </Td>
                    )}
